...
import { TICKET_CATEGORY_OPTIONS, TICKET_SOURCE_OPTIONS, withCurrentOption } from '@/lib/form-options';
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <FormSelectField id="category" label="Category" value={data.category} onChange={(value) => setData('category', value)} options={withCurrentOption(TICKET_CATEGORY_OPTIONS, data.category)} placeholder="Select category" error={errors.category} />
          <FormField id="title" label="Title" error={errors.title} className="md:col-span-2"><Input id="title" value={data.title} onChange={(e) => setData('title', e.target.value)} placeholder="Brief summary of the issue" /></FormField>
          <FormField id="description" label="Description" error={errors.description} className="md:col-span-2"><Textarea id="description" rows={5} value={data.description} onChange={(e) => setData('description', e.target.value)} placeholder="Describe the issue, steps taken, and expected outcome" /></FormField>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <FormSelectField id="source" label="Source" value={data.source} onChange={(value) => setData('source', value)} options={withCurrentOption(TICKET_SOURCE_OPTIONS, data.source)} placeholder="Select source" error={errors.source} />
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <FormSelectField id="priority" label="Priority" value={data.priority} onChange={(value) => setData('priority', value)} options={priorityOptions} placeholder="Select priority" error={errors.priority} />
          ...
          <FormField id="impact" label="Impact (1-5)" error={errors.impact}><Input id="impact" type="number" min="1" max="5" value={data.impact} onChange={(e) => setData('impact', e.target.value)} placeholder="1 to 5" /></FormField>
          <FormField id="urgency" label="Urgency (1-5)" error={errors.urgency}><Input id="urgency" type="number" min="1" max="5" value={data.urgency} onChange={(e) => setData('urgency', e.target.value)} placeholder="1 to 5" /></FormField>
