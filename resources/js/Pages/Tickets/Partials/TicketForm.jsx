    ...
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2"><Label>Service</Label><select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={data.service_id} onChange={(e) => setData('service_id', e.target.value)}><option value="">No linked service</option>{services.map((service) => <option key={service.id} value={service.id}>{service.name}</option>)}</select><p className="text-xs text-destructive">{errors.service_id}</p></div>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-2"><Label>SLA plan override</Label><select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={data.sla_plan_id} onChange={(e) => setData('sla_plan_id', e.target.value)}><option value="">Use service/client SLA</option>{formData.slaPlans.map((plan) => <option key={plan.id} value={plan.id}>{plan.name}</option>)}</select><p className="text-xs text-destructive">{errors.sla_plan_id}</p></div>
