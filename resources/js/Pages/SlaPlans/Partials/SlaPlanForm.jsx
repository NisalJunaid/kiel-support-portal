    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <Card>
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2"><Label>Name</Label><Input value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="Enter SLA plan name" /><p className="text-xs text-destructive">{errors.name}</p></div>
          <div className="space-y-2"><Label>Response minutes</Label><Input type="number" min="1" value={data.response_minutes} onChange={(e) => setData('response_minutes', e.target.value)} placeholder="e.g. 30" /><p className="text-xs text-destructive">{errors.response_minutes}</p></div>
          <div className="space-y-2"><Label>Resolution minutes</Label><Input type="number" min="1" value={data.resolution_minutes} onChange={(e) => setData('resolution_minutes', e.target.value)} placeholder="e.g. 240" /><p className="text-xs text-destructive">{errors.resolution_minutes}</p></div>
