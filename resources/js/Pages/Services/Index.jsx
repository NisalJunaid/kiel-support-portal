...
export default function ServicesIndex({ services, filters, can, domainReferences, clients, formData }) {
  ...
  const { data, setData, post, processing, errors, reset } = useForm({ client_company_id: formData.defaultClientId || '', name: '', service_type: 'managed_hosting', status: 'operational', sla_plan_id: '', renewal_cycle: '', start_date: '', renewal_date: '', end_date: '', notes: '', asset_ids: [], from_drawer: true });
  ...
  return (
    <AppLayout title="Services" description="Manage client services/subscriptions and linked assets." breadcrumbs={[{ label: 'Home', href: '/dashboard' }, { label: 'Services' }]}> 
      <Card>
        <CardHeader className="flex-row items-center justify-between space-y-0"><CardTitle>Services</CardTitle>{can.create && <Button size="sm" onClick={() => setCreateOpen(true)}>Create service</Button>}</CardHeader>
        <CardContent className="space-y-4">
          ...
          {services.data.length === 0 ? <EmptyState title="No services found" description="Create your first service/subscription or adjust the filters." /> : <><Table><TableHeader><TableRow><TableHead>Service</TableHead><TableHead>Client</TableHead><TableHead>Type</TableHead><TableHead>Status</TableHead><TableHead>Linked assets</TableHead><TableHead>Renewal</TableHead><TableHead /></TableRow></TableHeader><TableBody>{services.data.map((service) => <TableRow key={service.id}><TableCell><Link href={`/services/${service.id}`} className="font-medium hover:underline">{service.name}</Link></TableCell><TableCell>{service.client?.name || '—'}</TableCell><TableCell><Badge variant="outline">{service.service_type}</Badge></TableCell><TableCell><DomainStatusBadge domainReferences={domainReferences} referenceKey="serviceStatus" value={service.status} /></TableCell><TableCell>{service.assets?.length ?? 0}</TableCell><TableCell>{service.renewal_date || '—'}</TableCell><TableCell className="text-right"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="outline" size="sm">Actions</Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => router.visit(`/services/${service.id}`)}>View</DropdownMenuItem>{can.update && <DropdownMenuItem onSelect={() => router.visit(`/services/${service.id}/edit`)}>Edit</DropdownMenuItem>}{can.delete && <DropdownMenuItem onSelect={() => { if (confirm('Archive this service?')) router.delete(`/services/${service.id}`); }}>Archive</DropdownMenuItem>}</DropdownMenuContent></DropdownMenu></TableCell></TableRow>)}</TableBody></Table><ListPagination paginated={services} /></>}
        </CardContent>
      </Card>

      <EntityCreateDrawer open={createOpen} onOpenChange={setCreateOpen} onCancel={() => { setCreateOpen(false); reset(); }} title="Create service" description="Create a support service for a client account." processing={processing}>
        <ServiceForm data={data} setData={setData} errors={errors} processing={processing} formData={formData} domainReferences={domainReferences} submitLabel="Create service" onSubmit={(e) => { e.preventDefault(); post('/services', { preserveScroll: true, onSuccess: () => { setCreateOpen(false); reset(); } }); }} />
      </EntityCreateDrawer>
    </AppLayout>
  );
}
