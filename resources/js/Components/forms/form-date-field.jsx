...
export function FormDateField({ id, label, value, onChange, error, hint, required = false }) {
  return (
    <FormField id={id} label={label} required={required} error={error} hint={hint}>
      <div className="relative">
        <CalendarDays className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
        <Input id={id} type="date" className="pl-9 pr-10" value={value || ''} onChange={(event) => onChange(event.target.value)} />
        {value ? (
