...
- Branding settings now use a normalized schema (`app_name`, `logo_path`/`logo_url`, `primary_color`, `secondary_color`, `accent_color`, `card_border_color`, `dark_mode_enabled`) persisted in `app_settings`; legacy `border_color`/`surface_border_color` values are auto-migrated at read time for backward compatibility.
